...
import 'package:my_drug_info_app/screens/settings_screen.dart';
...
import 'chinese_drug_screen.dart'; // Import the Chinese Drug Screen
...
class _DrugScreenState extends State<DrugScreen> {
  ...
  void _searchDrugInfo() async {
    ...
    try {
      final drugs = await DBHelper.getDrugByName(drugName);
      setState(() {
        _filteredDrugInfo = drugs
            .where((drug) => drug.name.toLowerCase().startsWith(drugName))
            .toList();
        _errorMessage =
            _filteredDrugInfo.isEmpty ? 'No drugs found with that name' : '';
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      drawer: _buildDrawer(),
      appBar: AppBar(
        title: const Text('Western Medicine'),
        centerTitle: true,
        actions: [
          IconButton(
        ...
        children: [
          ...
          _buildDrawerItem('Chinese', Icons.medication_liquid, () {
            ...
            Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => ChineseDrugScreen(
                        // Navigate to ChineseDrugScreen
                        onThemeChanged: widget.onThemeChanged,
                        onLanguageChanged: widget.onLanguageChanged,
                      )),
            );
